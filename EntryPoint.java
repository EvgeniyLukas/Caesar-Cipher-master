...
public class EntryPoint {
	...
	void startEncrypt(){
		...
		if(!encrFile.exists())
			try { encrFile.createNewFile(); System.out.println("Файл успішно створен."); }
		catch (IOException e) { throwError(create); }
		
		String degOP = JOptionPane.showInputDialog(null, "Введіть потрібний зсув:", "Зсув", JOptionPane.QUESTION_MESSAGE);
		int deg = 0;
	...
	void startDecrypt(){
		...
		if(!decrFile.exists())
			try { decrFile.createNewFile(); System.out.println("Файл успішно створен."); }
		catch (IOException e) { throwError(create); }
		...
		frameDecr.decrText.setText("Зсув 1: " + crypt.exp_decr);
			...
			public void mouseClicked(MouseEvent e) {
				if(EntryPoint.deg < crypt.alphabet.length-1){
					...
					if(!frameDecr.checkMostDegBox.isSelected()){
						...
						frameDecr.decrText.setText("Зсув " +  EntryPoint.deg + ": " + crypt.exp_decr);
					}
				}
			};
		};
		...
		for(int i = 0; i < exps.length; i++){
			try{
				...
				if(crypt.count[s] == success) { 
					...
					frameDecr.most_popular_degs.addItem("Зсув " + deg + ". Вдалих: " + success);
					indexForMenu++;
				}
	...
	void report(){
		...
		if(!reportFile.exists())
			try { reportFile.createNewFile(); System.out.println("Файл статистики успішно створен."); }
		catch (IOException e) { throwError(create); }
	...
	void throwError(String err){
		String text = "";
		switch(err) {
		case "create": text = "Не вдалось створити файл."; break;
		case "write": text = "Не вдалось записати у файл."; break;
		case "digit": text = "Необхідно ввести цифру"; break;
		case "read": text = "Не вдалось прочитати файл"; break;
		case "nullm": text = "Порожне меню успішных здвигів"; break;
		default: text = "Невідома помилка"; break;
		}
		...
		JOptionPane.showMessageDialog(null, text, "Помилка", JOptionPane.ERROR_MESSAGE);
